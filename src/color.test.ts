...
describe("resolveBasePalette", () => {
  ...
  test("throws a clear error when a shade string cannot be parsed", () => {
    // Arrange
    const shades: [string, string, string] = ["not-a-color", "#a78bfa", "#818cf8"];

    // Act & Assert
    expect(() => resolveBasePalette({ shades })).toThrow(/could not parse shade color/);
  });
...
describe("driftColor", () => {
  test("leaves hue/lightness/chroma unchanged when hueShift and intensity are 0", () => {
    ...
    // Act
    const drifted = driftColor({ base, time: 5, hueShift: 0, intensity: 0, offset: 0 });
  ...
  test("shifts hue over time proportionally to hueShift", () => {
    ...
    // Act
    const drifted = driftColor({ base, time: 1, hueShift: 1, intensity: 0, offset: 0 });
